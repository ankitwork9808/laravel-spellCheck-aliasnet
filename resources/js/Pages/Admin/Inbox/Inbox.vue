<template>
    <inner-layout :title="page.company_name">

        <div class="bg-primary-orange inbox" id="inboxItems">
            <div class="m-3">
                <label class="form-label text-white" for="search">{{page.company_name}}</label>
                <div class="input-group">
                    <input type="text" @keyup="searchCompany($event)" class="search-company form-control" placeholder="🔍">
                </div>
            </div>
            <div class="list-group list-group-flush scrollarea">
                <template v-for="(company, index) in page.companies" :key="index">
                    <div @click="getCases(company.id, company.name, true, 'main')" class="list-group-item list-group-item-action cursor-pointer">
                        <div class="d-flex w-100 align-items-center justify-content-between">
                            <p class="mb-1">{{ company.name }}</p>
                            <small>{{ company.cases_count }}</small>
                        </div>
                    </div> 
                </template>
            </div>
        ...
        <div class="col-sm-12" id="companyCase">
            <div class="row">
                <div class="col-sm-6">
                    ...
                    <div @scroll="onScroll" class="list-group list-group-flush scrollarea h-85vh" id="inboxitems">
                        <template v-for="(object, index) in page.cases" :key="index">
                            <div @click="readCase(object)" :id="object.case_number" :class="'list-group-item cursor-pointer case-row list-group-item-action py-3 '+object.selected">
                                  ...
                                  <div class="">
                                    <p class="mb-1">
                                        <template v-if="object.seen">
                                           <span>{{ lang.case_id }}:</span> <span class="font-weight-6">{{object.case_number}}</span>
                                        </template>
                                        <template v-else>
                                            <strong>{{ lang.case }} {{object.case_number}}</strong>
                                        </template>

                                        <i v-if="! object.deleted_at" data-bs-toggle="tooltip" data-bs-placement="top" :title="lang.archive" class="fa fa-archive text-1 ml-4 cursor-pointer" @click="softDelete(object.id)">
                                        </i>
                                ...
                                <ul class="flex-wrap unstyled inline-flex p-0 mb-0 text-blue-700">
                ...
                <div v-if="page.case.id > 0" class="col-sm-6">
                    <div class="d-flex justify-content-between border-bottom align-items-center py-3">
                    ...
                    <div class="d-flex flex-column h-85vh scrollarea">
                        ...
                        <div class="mb-3 ps-2">
                            ...
                            <template v-if="page.case.notes.length">
                                <div v-for="(note, index) in page.case.notes" :key="index">
                                    <template v-if="note.is_private && note.user.id == $page.props.user.id">
                                        <p class="pt-3 text-black-50 d-block">
                                            {{ note.note }}
                                            ({{ lang.private }})
                                        </p>
                                        <p class="text-end text-black-50 d-block font-weight-6">
    ...
    export default defineComponent({
        ...
        methods:{
            ...
            getCases(company_id, company_name, mode, from){
                ...
                axios.post(route('cases.cases', company_id), data).then(response => {
                    ...
                    if(mode){
                        let data = new FormData
                        data.append('keyword', '')
                        axios.post(route('company.search'), data).then(response => {
                            this.page.companies = response.data;
                        });
                    }

                    /* keep it here, in the last line of this method  */
                    this.page.cases.map(obj => {
                        if(obj.case_number == this.page.from_dashboard_case.case_number){
                            this.readCase(obj);
                        }
                    });
                });
            ...
            readCase(object){
                ...
                axios.get(route('case.count', object.id)).then(response => {
            ...
            edit(){
                ...
                axios.get(route('manager.list', this.page.company_id)).then(response => {
            ...
            addNote(case_id){
                ...
                axios.post(route('notes.store'), data).then(response => {
            ...
            updateStatus(object){
                ...
                axios.post(route('case.status'), data).then(response => {
            ...
            updateCategory(case_id){
                ...
                axios.post(route('notes.category'), data).then(response => {
            ...
            updateManager(object){
                ...
                axios.post(route('case.manager'), data).then(response => {
            ...
            searchCompany(event){
                ...
                let keyword = $(event.srcElement).val();
                let data = new FormData
        ...
        mounted(){
            ...
            this.getCases(this.companies[0].id, this.companies[0].name, false, 'main');
            this.page.company_id = this.companies[0].id;

            if(this.case_num){

                axios.get(route('case.detail', this.case_num)).then(response => {
