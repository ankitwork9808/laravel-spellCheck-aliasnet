<template>
    <Head :title="$page.props.common.two_factor_confirmation" />
    ...
    <jet-authentication-card>
        ...
        <div class="mb-4 text-sm text-gray-600">
            <template v-if="! recovery">
                {{ $page.props.common.please_confirm_access_to_your_account }}
            </template>

            <template v-else>
                {{ $page.props.common.entering_one_of_your_emergency }}
            </template>
        </div>
        ...
        <form @submit.prevent="submit">
            ...
            <div class="flex items-center justify-end mt-4">
                <button type="button" class="text-sm text-gray-600 hover:text-gray-900 underline cursor-pointer" @click.prevent="toggleRecovery">
                    <template v-if="! recovery">
                        {{ $page.props.common.use_recovery_code }}
                    </template>

                    <template v-else>
                        {{ $page.props.common.use_authentication_code }}
                    </template>
                </button>

                <jet-button class="ml-4" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                    {{ $page.props.common.login }}
                </jet-button>
            </div>
        </form>
    </jet-authentication-card>
